...
CUSTOM_ACTIVITY = {
    ...
    "items": [
        {"category": "Pakaian", "name": "Yoga mat", "quantity": 1, "is_essential": True, "priority": 8, "gender_filter": "all"},
        {"category": "Pakaian", "name": "Yoga pants", "quantity": 2, "is_essential": False, "priority": 5, "gender_filter": "all"},
    ],
}
...
async def test_add_item_to_custom_activity(client):
    ...
    r = await client.post(f"/activities/{activity['id']}/items", json={
        "category": "Lainnya", "name": "Foam roller", "quantity": 1,
