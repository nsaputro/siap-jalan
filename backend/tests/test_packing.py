...
async def test_add_item_to_list(client):
    ...
    r = await client.post(f"/lists/{list_id}/items", json={"name": "Passport", "category": "Dokumen", "quantity": 1, "is_essential": True})
    assert r.status_code == 201
    item = r.json()
...
async def test_add_adhoc_item_with_source_activity(client):
    ...
    r = await client.post(f"/lists/{list_id}/items", json={
        "name": "Blister kit", "category": "Obat-obatan", "quantity": 1,
...
async def test_toggle_item_unpacked_to_packed(client):
    _, list_id = await _create_trip(client)
    item = (await client.post(f"/lists/{list_id}/items", json={"name": "Sunscreen", "category": "Lainnya", "quantity": 1, "is_essential": False})).json()
...
async def test_toggle_item_packed_to_unpacked(client):
    _, list_id = await _create_trip(client)
    item = (await client.post(f"/lists/{list_id}/items", json={"name": "Hat", "category": "Pakaian", "quantity": 1, "is_essential": False})).json()
...
async def test_update_item_fields(client):
    _, list_id = await _create_trip(client)
    item = (await client.post(f"/lists/{list_id}/items", json={"name": "Old name", "category": "Lainnya", "quantity": 1, "is_essential": False})).json()
...
async def test_delete_item(client):
    _, list_id = await _create_trip(client)
    item = (await client.post(f"/lists/{list_id}/items", json={"name": "Toothbrush", "category": "Toilet & Kebersihan", "quantity": 1, "is_essential": False})).json()
...
async def test_bulk_create_items(client):
    _, list_id = await _create_trip(client)
    payload = [
        {"list_id": list_id, "name": "Item A", "category": "Lainnya", "quantity": 1, "is_essential": False, "added_by": "user", "source_activities": []},
        {"list_id": list_id, "name": "Item B", "category": "Lainnya", "quantity": 2, "is_essential": True,  "added_by": "user", "source_activities": []},
        {"list_id": list_id, "name": "Item C", "category": "Dokumen",  "quantity": 1, "is_essential": True,  "added_by": "user", "source_activities": []},
    ]
